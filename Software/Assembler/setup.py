...
- Ensures the executable is at: Software/Assembler/bin/HackAssembler.exe
...
Usage (from repo root, e.g., CPU-Design-in-Logisim):
    python .\Software\Assembler\setup_env.py
Optional:
...
def do_build(msys_dir: pathlib.Path, Assembler_dir: pathlib.Path) -> None:
    c_ms = win_to_msys(Assembler_dir)
...
def normalize_binary(Assembler_dir: pathlib.Path) -> pathlib.Path:
    """
    Place final exe in: Software/Assembler/bin/HackAssembler.exe
    Accepts legacy names and locations, and moves/renames as needed.
    """
    bin_dir = Assembler_dir / "bin"
    bin_dir.mkdir(parents=True, exist_ok=True)

    targets = [
        Assembler_dir / "HackAssembler.exe",
        Assembler_dir / "HackAssembler",
        Assembler_dir / "HackComp.exe",
        Assembler_dir / "HackComp",
...
def main() -> None:
    ...
    Assembler_dir = proj_root / "Software" / "Assembler"

    if not Assembler_dir.exists():
        fail(f"Assembler directory not found: {Assembler_dir}")
    ...
    do_build(msys_dir, Assembler_dir)
    normalize_binary(Assembler_dir)

    print("[OK] Setup complete. You can now run: Software\\Assembler\\bin\\run.bat")
