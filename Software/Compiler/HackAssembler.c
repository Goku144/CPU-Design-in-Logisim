...
static int write_all(const char *path, const char *buf, size_t n) {
    ...
    if (n && fwrite(buf, 1, n, fp) != n) { perror("fwrite"); fclose(fp); return -1; }
    fclose(fp);
    return 0;
}
...
static int options(int argc, const char *argv[])
{
    ...
    if (strcmp(opt, "-o") == 0) {
        ...
        if (!p_hack) { perror("malloc"); free(bin.str); return 1; }
        ...
        if (!hx.str) { fprintf(stderr, "convToHex failed\n"); free(bin.str); return 1; }
        size_t hx_bytes = (hx.size ? hx.size - 1 : 0);

        int need = snprintf(NULL, 0, HEX_FMT, out);
        char *p_hex = (char*)malloc((size_t)need + 1);
        if (!p_hex) { perror("malloc"); free(hx.str); free(bin.str); return 1; }
        ...
        if (!p_hack || !p_hex) { perror("malloc"); free(p_hack); free(p_hex); free(hx.str); free(bin.str); return 1; }

        snprintf(p_hack, (size_t)need_h + 1, HACK_FMT, out);
        snprintf(p_hex,  (size_t)need_x + 1, HEX_FMT,  out);

        rc = write_all(p_hack, bin.str, bin_bytes);
        if (rc == 0) rc = write_all(p_hex, hx.str, hx_bytes);
